...
description: Interview-style creation of a single Prism instrument (lens, frame, model, stance, or heuristic). Triggers on "create a new lens for X", "add a framework for Y", "make an instrument for Z", "새 렌즈 만들어줘", "프레임 추가해줘". Collects minimal input, disambiguates the target class via CLASSES.md, then generates one file via an LLM call and saves it to the project or global Prism layer. Do NOT use for batch generation (scripts/prism_batch.py) or for browsing the catalog (catalog-browse).
---
...
file-format templates. This skill references CLASSES.md rather than
...
- **Batch generation** (many instruments at once) → use
  ...
  This skill creates exactly one file per invocation.
