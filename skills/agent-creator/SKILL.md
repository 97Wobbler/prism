...
You are building a **instrument-composing analysis agent** — a lightweight
agent that does rigorous domain-expert analysis by loading structured
instruments, not by pretending to be a persona.

## Core principle

Persona prompts ("You are a security expert") change tone but not
reasoning depth. Structured instruments (STRIDE, OWASP Top 10, Cynefin,
...
**v0.2 introduces 5 classes of instrument.** Not every useful instrument is
a "lens" in the strict sense. Your job is to pick the right *class* for
each instrument you add, compose them into a minimal agent config, and
...
- **Stance (비판적 관점)** — interpretive position. Applied in the
  `critical-read` step to surface what other instruments miss. E.g.,
...
- The source instrument has a procedure, an output template, and a
...
- Don't write persona prompts full of adjectives. The instruments do
  the work.
