...
python3 scripts/lenslab_batch.py \
  --workers 5 \
  --batch-size 10 \
  --usage-limit 85
```
...
### 로그 파일로 남기기
```bash
python3 scripts/lenslab_batch.py --workers 5 --batch-size 10 --usage-limit 85 \
  2>&1 | tee -a logs/phase3-$(date +%Y%m%d-%H%M).log
```

### 테스트용 소규모 실행
```bash
python3 scripts/lenslab_batch.py --limit 20 --workers 2
```

### 특정 클래스만 (예: heuristic만 62개)
```bash
python3 scripts/lenslab_batch.py --class heuristic --workers 3
```
...
중단 후 같은 명령을 재실행하면:
1. `parallel_runner`가 `.batch-state/lenslab-run/items/*.json`을 보고 이미 처리된 항목 스킵
2. `lenslab_batch`도 대상 md 파일이 이미 존재하면 LLM 호출 없이 스킵
...
2. **실패 항목 확인**:
   ...
   import json, glob
   fails = []
   for f in glob.glob(".batch-state/lenslab-run/items/*.json"):
       with open(f) as fh:
           r = json.load(fh)
       if r.get("_error"):
...
해당 아이템만 격리해서 재시도:
```bash
python3 scripts/lenslab_batch.py --class lens --limit 1 \
...
`.batch-state/lenslab-run/items/<name>.json`의 `body_preview` 필드 확인.
프롬프트 튜닝이 필요할 수 있음.
